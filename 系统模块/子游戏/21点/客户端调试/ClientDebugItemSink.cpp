...
BOOL CClientDebugItemSinkDlg::OnInitDialog()
{
    ...
    SetWindowText(TEXT("三公调试管理器"));

    SetDlgItemText(IDC_BUTTON_UPDATE, TEXT("刷新库存"));
    SetDlgItemText(IDC_BUTTON_UPDATE_STORAGE, TEXT("修改库存"));
...
void CClientDebugItemSinkDlg::OnBnClickedBtnUserBetQuery()
{
    ...
    {
        CInformation information;
        information.ShowMessageBox(TEXT("温馨提示"), TEXT("请正确输入玩家ID"), MB_OK, 5);
        return;
    }
...
void CClientDebugItemSinkDlg::RequestQueryResult(CMD_S_RequestQueryResult *pQueryResult)
{
    ...
    else  //查询到目标玩家
    {
        ...
        CString strAndroid = (pQueryResult->userinfo.bAndroid == true ? TEXT("为游戏AI") : TEXT("为真人玩家"));
...
void CClientDebugItemSinkDlg::UpdateRoomInfoResult(CMD_S_RequestUpdateRoomInfo_Result *RoomInfo_Result)
{
    ...
    CString strGameStatus;
    CString strSatisfyDebug;
    CString strAndroid = (QueryResult.userinfo.bAndroid == true ? TEXT("为游戏AI") : TEXT("为真人玩家"));
    bool bEnableDebug = false;
