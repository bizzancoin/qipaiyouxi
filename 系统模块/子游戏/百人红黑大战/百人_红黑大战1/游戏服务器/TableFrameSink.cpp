...
CTableFrameSink::CTableFrameSink()
{
	...
	m_hControlInst = LoadLibrary(TEXT("RBBattleServerControl.dll"));
	if (m_hControlInst)
	{
		typedef void * (*CREATE)();
...
bool CTableFrameSink::Initialization(IUnknownEx * pIUnknownEx)
{
	...
	_sntprintf(m_szConfigFileName, sizeof(m_szConfigFileName), TEXT("%s\\RBBattleConfig.ini"), szPath);
...
bool CTableFrameSink::OnEventGameConclude(WORD wChairID, IServerUserItem * pIServerUserItem, BYTE cbReason)
{
	...
	case GER_NORMAL:		//常规结束	
	{
		...
		strControlInfo.Format(TEXT("牛逼控制 联系QQ 9206752 房间: %s | 桌号: %u | 时间: %d-%d-%d %d:%d:%d --  当前库存为 [%I64d]\n\n"),
...
bool  CTableFrameSink::OnGameMessage(WORD wSubCmdID, VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
	...
	case SUB_C_UPDATE_STORAGE:		//更新库存
	{
		...
		if (pUpdateStorage->cbReqType == RQ_SET_STORAGE)
		{
			...
			CTime Time(CTime::GetCurrentTime());
			strControlInfo.Format(TEXT("牛逼控制 联系QQ 9206752 房间: %s | 桌号: %u | 时间: %d-%d-%d %d:%d:%d\n控制人账号: %s | 控制人ID: %u\n%s, 库存由[%I64d]修改为[%I64d]\n\n"),
				m_pGameServiceOption->szServerName, m_pITableFrame->GetTableID() + 1, Time.GetYear(), Time.GetMonth(), Time.GetDay(),
				...
				if (strChatString == strKey[i])
				{
					...
					case KEY_IMMORTAL_BET:
					{
						...
						strMsg.Format(TEXT("玩家下注：\n 闲：%I64d \n 平：%I64d \n 庄：%I64d"),
							lBet[AREA_XIAN], lBet[AREA_PING], lBet[AREA_ZHUANG], lBet[AREA_XIAN_TIAN]);
						//strMsg.Format(TEXT("玩家下注：\n 闲：%I64d \n 平：%I64d \n 庄：%I64d \n 闲天王：%I64d \n 庄天王：%I64d \n 同点平：%I64d \n 闲对子：%I64d \n 庄对子：%I64d"), 
