...
BEGIN
	...
	BEGIN
		...
		@CellOnlinePresent=CellOnlinePresent,@CellOnlineTime=CellOnlineTime,@StartOnlineTime=StartOnlineTime,
		@IsPlayPresent=IsPlayPresent,@IsOnlinePresent=IsOnlinePresent
		FROM RYPlatformDBLink.RYPlatformDB.dbo.GlobalPlayPresent WHERE ServerID=-2
		IF @ServerID IS NULL
		BEGIN
			SET @IsPlayPresent=0
	...
	BEGIN
		...
		SELECT @UserDatePresent=PresentScore FROM RYTreasureDBLink.RYTreasureDB.dbo.StreamPlayPresent WHERE DateID=@DateID AND UserID=@dwUserID
	...
	BEGIN
		...
		SELECT @UserPresent=SUM(PresentScore) FROM RYTreasureDBLink.RYTreasureDB.dbo.StreamPlayPresent WHERE UserID=@dwUserID
	...
	BEGIN		
		-- 更新用户金币
		UPDATE RYTreasureDBLink.RYTreasureDB.dbo.GameScoreInfo set Score=Score+@PresentScore WHERE UserID=@dwUserID

		UPDATE RYTreasureDBLink.RYTreasureDB.dbo.StreamPlayPresent SET PresentCount=PresentCount+1,PresentScore=PresentScore+@PresentScore,
		...
		IF @@ROWCOUNT=0
		BEGIN
			INSERT RYTreasureDBLink.RYTreasureDB.dbo.StreamPlayPresent (DateID,UserID,PresentCount,PresentScore,PlayTimeCount,OnLineTimeCount)
