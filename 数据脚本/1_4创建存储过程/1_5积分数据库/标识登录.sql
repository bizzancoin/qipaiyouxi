...
--	END
	...
	SELECT @Insure=InsureScore FROM RYTreasureDBLink.RYTreasureDB.dbo.GameScoreInfo WHERE UserID=@dwUserID
...
BEGIN
	...
	-- 分数信息	
	IF @cbPersonalServer=1
	BEGIN
		UPDATE GameScoreInfo SET Score=0, WinCount=0, LostCount=0, DrawCount=0, FleeCount=0 WHERE UserID=@dwUserID
		SELECT @Score=Score, @WinCount=WinCount, @LostCount=LostCount, @DrawCount=DrawCount, @FleeCount=FleeCount FROM GameScoreInfo WHERE UserID=@dwUserID
		--SELECT @Score=Score, @WinCount=WinCount, @LostCount=LostCount, @DrawCount=DrawCount, @FleeCount=FleeCount FROM PersonalServerScoreInfo
		--WHERE UserID=@dwUserID AND ServerID=@wServerID
	...
	BEGIN
		...
		DECLARE @LockKindID INT
		DECLARE @LockServerID INT
		SELECT @LockKindID=KindID, @LockServerID=ServerID FROM RYTreasureDBLink.RYTreasureDB.dbo.GameScoreLocker WHERE UserID=@dwUserID AND ServerID=@wServerID
		...
		BEGIN
			...
			BEGIN
				-- 查询信息
				SET @strErrorDescribe=N'您的账号正在别的约战房中游戏！'
				RETURN 4
	...
	BEGIN
		...
		BEGIN
			...
			DELETE RYTreasureDBLink.RYTreasureDB.dbo.GameScoreLocker WHERE UserID=@dwUserID AND ServerID=@wServerID
			INSERT RYTreasureDBLink.RYTreasureDB.dbo.GameScoreLocker (UserID, ServerID, KindID, EnterID, EnterIP, EnterMachine) VALUES (@dwUserID, @wServerID, @wKindID, @InoutIndex, @strClientIP, @strMachineID)
	...
	IF @cbPersonalServer=1
	BEGIN
		DECLARE @PersonalScore BIGINT
		--SELECT @PersonalScore=IniScore
	END
