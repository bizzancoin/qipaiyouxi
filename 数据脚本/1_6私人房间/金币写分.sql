...
BEGIN
	...
	BEGIN
		...
		INSERT INTO RecordInsure(KindID,ServerID,SourceUserID,SourceGold,SourceBank,
			SwapScore,Revenue,IsGamePlaza,TradeType,ClientIP, CollectNote)
		VALUES(@wKindID,@wServerID,@dwRoomHostID,@SourceScore ,@InsureScore,0, @TaxCount,0,1,@strClientIP, N'房卡税收')	


		-- 更新房卡记录中的税收
		UPDATE RYPlatformDBLink.RYPlatformDB.dbo.StreamCreateTableFeeInfo SET TaxRevenue = TaxRevenue + @TaxCount WHERE UserID = @dwRoomHostID AND RoomID = @szRoomID
