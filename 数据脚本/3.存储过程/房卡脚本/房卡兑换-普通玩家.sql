...
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].NET_PW_ExchRoomCardGeneral') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].NET_PW_ExchRoomCardGeneral
GO
...
CREATE PROCEDURE NET_PW_ExchRoomCardGeneral
	@dwUserID	INT,						-- 用户 I D
...
BEGIN
	...
	-- 获取兑换比例
	SELECT @Rate = UseResultsGold FROM RYPlatformDBLink.RYPlatformDB.dbo.GameProperty WHERE ID = 501
	...
	SELECT @UserGold=InsureScore FROM GameScoreInfo WHERE UserID=@UserID
	IF @UserGold IS NULL
	BEGIN
		SET @UserGold = 0
		INSERT INTO GameScoreInfo (UserID, InsureScore, RegisterIP, LastLogonIP) VALUES (@UserID, @Gold, @RegisterIP, @RegisterIP) 
	END
	ELSE
	BEGIN
