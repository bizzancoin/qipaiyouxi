...
BEGIN
	-- 验证用户
	SELECT @Score=Score,@UserID=UserID FROM RYTreasureDBLink.RYTreasureDB.dbo.GameScoreInfo 
	WHERE UserID=@UserID
	IF @UserID IS NULL
	BEGIN
	...
	-- 更新奖牌
	--BEGIN TRAN
	UPDATE RYTreasureDBLink.RYTreasureDB.dbo.GameScoreInfo SET sScore=sScore-@TotalAmount
	WHERE UserID=@UserID
