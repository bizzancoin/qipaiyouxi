...
(function ($){
    ...
    var fv_scriptSrc = getScriptAbsoluteSrc(scripts[scripts.length - 1]);
    ...
    var initConfig_setting = {
        ...
        mode: "AutoTip",  //显示模式。固定提示层FixTip|自动构建提示层AutoTip|单个提示层跟随SingleTip|弹出提示内容AlertTip
        errorFocus: true,  //第一个错误的控件获得焦点
        wideWord: true,  //一个汉字当做2个长
        forceValid: false,  //控件输入正确之后，才允许失去焦
        debug: false,  //调试模式点
        inIframe: false,
        onSuccess: $.noop,  //提交成功后的回调函数
    ...
    var inputValidator_setting = {
        ...
        trim: {
            trimLeft: false,
            trimRight: false,
            trimLeftError: "字符左端不能包含空格",
            trimRightError: "字符右端不能包含空格"
        }
    };
    ...
    var passwordValidator_setting = {
        ...
        onErrorCompareSame: "密码于帐号相同不被允许"
    };
    ...
    $.formValidator = {
        ...
        initConfig: function (controlOptions){
            ...
            if (settings.formID) {
                $("#" + settings.formID).submit(function (e){
                    if (settings.ajaxForm) {
                        ...
                        if (!$.formValidator.pageIsValid(settings.validatorGroup)) {
                            return e.preventDefault();
                        }
                    }
                });
            ...
            $(doc.body).data("data-fv-group-" + settings.validatorGroup, settings);
            ...
            $.ajax({
                async: false,
                type: "GET",
                url: themedir + "themes/" + settings.theme + "/js/theme.js", dataType: "script",
                error: function (){
                    throw new Error('Load theme error');
                }
            });
            ...
            baseElement ? head.insertBefore(css, baseElement) : head.appendChild(css);

            $(window).bind('resize.fv', function (){
                clearTimeout($(doc.body).data('data-fv-nexttick'));
                $(doc.body).data('data-fv-nexttick', setTimeout(function (){
                    $.formValidator.reloadAutoTip(settings.validatorGroup);
                }, 100));
            });
        },
        ...
        sustainType: function (elem, validateType){
            ...
            switch (validateType) {
                ...
                case "ajaxValidator":
                    return (stype === "text" || stype === "textarea" ||
                        stype === "file" || stype === "password" || stype === "select-one");
                case "regexValidator":
                    return ((srcTag === "INPUT" || srcTag === "TEXTAREA") ? (stype !== "checkbox" && stype !== "radio") : false);
        ...
        appendValid: function (id, setting){
            ...
            //重新初始化
            if (validateType === "formValidator" || !elem.settings) {
                elem.settings = [];
            }
        ...
        setTipState: function (elem, showclass, showmsg){
            var target = $(elem);
            var initConfig = $(doc.body).data("data-fv-group-" + elem.validatorGroup);

            if (initConfig.mode === "SingleTip") {
                //显示和保存提示信息
                var singleTip = $("#fv-singletip-content");
                showmsg = '<i class="' + showclass + '"></i><span>' + showmsg + '</span>';
                singleTip.html(showmsg);
                target.data('data-message', showmsg);
            } else {
                var tip = $("#" + elem.settings[0].tipID);
                var html = showclass === "onShow" ? onShowHtml :
                    (showclass === "onFocus" ? onFocusHtml :
                        (showclass === "onCorrect" ? onCorrectHtml : onErrorHtml));
                ...
                if (!html) {
                    ...
                    if (elem.validatorPasswordIndex > 0 && showclass === "onCorrect") {
                        ...
                        showmsg = window.passwordStrengthText[level <= 0 ? 0 : level - 1];
                    }
                    ...
                    if (showclass) {
                        tip.html(html)
                            .removeClass()
                            .addClass('ui-fv-autotip ' + showclass).show();
                    } else {
                        tip.html(html).show();
                    }
                }
            ...
            if (stype === "password" || stype === "text" || stype === "file") {
                ...
                if (showclass === "onShow") {
                    target.removeClass(rclass).addClass(onShowClass);
                }

                if (showclass === "onFocus") {
                    target.removeClass(rclass).addClass(onFocusClass);
                }

                if (showclass === "onCorrect") {
                    target.removeClass(rclass).addClass(onCorrectClass);
                }

                if (showclass === "onError") {
                    target.removeClass(rclass).addClass(onErrorClass);
                }
            }
        },
        ...
        resetTipState: function (validatorGroup){
            ...
            var initConfig = $(doc.body).data("data-fv-group-" + validatorGroup);
            ...
            $.each(initConfig.validObjects, function (){
                ...
                $.formValidator.setTipState(
                    this, passed ? "onCorrect" : "onShow",
                    passed ? $.formValidator.getStatusText(this, setting.onCorrect) : setting.onShow
                );
            });
        },
        ...
        showMessage: function (returnObj){
            ...
            if (!isValid) {
                ...
                if (setting.validateType === "ajaxValidator") {
                    ...
                    showmsg = (!returnObj.errorMsg ?
                        $.formValidator.getStatusText(elem, setting.onError) : returnObj.errorMsg);
                }

                if (initConfig.mode == "AlertTip") {
                    if (elem.validValueOld !== $(elem).val()) {
                        initConfig.onAlert(showmsg);
                    }
                } else {
                ...
                //showmsg = $.formValidator.isEmpty(id) ?
                //    setting.onEmpty : $.formValidator.getStatusText(elem, setting.onCorrect);
                showmsg = $.formValidator.getStatusText(elem, setting.onCorrect);
        ...
        isRequiredValid: function (id){
            var required = $("#" + id).get(0).settings[0].required;
            return !(required && !$.formValidator.getLength(id));
        },
        ...
        oneIsValid: function (id){
            ...
            e.id = id;
            e.dom = $("#" + id).get(0);
            e.initConfig = $(doc.body).data("data-fv-group-" + e.dom.validatorGroup);
            e.ajax = -1;
            e.errorMsg = "";
            ...
            if (e.settings[0].required) {
                e.isValid = $.formValidator.isRequiredValid(id);
                e.setting = e.settings[0];
                if (!e.isValid) {
                    e.errorMsg = e.setting.onEmpty;
                    return e;
                }
            } else {
                if (!$.formValidator.getLength(id)) {
                    e.isValid = true;
                    e.setting = e.settings[0];
                    return e;
                }
            ...
            for (var i = 0; i < settingslen; i++) {
                ...
                //根据类型触发校验
                switch (e.settings[i].validateType) {
                    case "inputValidator":
                ...
                if (!e.settings[i].isValid) {
                    e.isValid = false;
                    e.setting = e.settings[i];
                    e.errorMsg = e.errorMsg || e.settings[i].onError;
                    break;
                } else {
                    e.isValid = true;
                    e.setting = e.settings[0];
        ...
        pageIsValid: function (validatorGroup){
            ...
            $.each(initConfig.validObjects, function (){
                ...
                if (this.settings[0].bind && !this.validatorAjaxIndex && !this.onceValided) {
                    ...
                    if (name_list.indexOf("^" + name + "^") === -1) {
                        ...
                        if (returnObj) {
                            ...
                            if (initConfig.mode !== "AlertTip") {
                                ...
                                !returnObj.isValid && initConfig.onAlert(errorMessage);
                            }
        ...
        ajaxForm: function (validatorGroup, option, formid){
            ...
            formid = formid || initConfig.formID;
            ...
            var form = $("#" + formid);
            var url = setting.url || form.attr("action");
            var data = $.param(setting.data || {});
        ...
        ajaxValid: function (e){
            ...
            initConfig = $(doc.body).data("data-fv-group-" + validatorGroup);

            var formdata = srcjo.serialize();
            //添加触发的控件名、随机数、传递的参数
            data = data ? formdata + "&" + data : formdata;
        ...
        regexValid: function (returnObj){
            ...
            if (elem.settings[0].empty && elem.value === "") {
                ...
                $.each(regexArray, function (i, r){
                    if (setting.dataType === "enum" && window.regexEnum) {
                        r = window.regexEnum[r];
                    }
        ...
        inputValid: function (returnObj){
            ...
            var trim = setting.trim, trimError = false;
            ...
            switch (sType) {
                ...
                case "file":
                    if (setting.type === "size") {
                        if (trim.trimLeft) {
                            trimError = val.replace(/^[ \s]+/, "").length !== val.length;
                            setting.isValid = !trimError;
                            if (trimError && trim.trimError) {
                                returnObj.errorMsg = trim.trimLeftError;
                                break;
                            }
                        }

                        if (!trimError && trim.trimRight) {
                            trimError = val.replace(/[ \s]+$/, "").length !== val.length;
                            setting.isValid = !trimError;
                            if (trimError && trim.trimError) {
                                returnObj.errorMsg = trim.trimRightError;
                                break;
                            }
                        }
                    }
        ...
        passwordValid: function (elem){
            ...
            var returnLevel = 0;
            var hasFind = true;
            $.each(window.passwordStrengthRule || [], function (j, n){
                var level = n.level;
                var rules = n.rule;
        ...
        reloadAutoTip: function (validatorGroup){
            ...
            $.each(initConfig.validObjects, function (){
                if (initConfig.mode === "AutoTip") {
                    ...
                    var tipCss = $.extend({}, setting.tipCss);
                    tipCss.top = offset.top + setting.tipCss.top;
                    tipCss.left = $relativeID.width() + offset.left + setting.tipCss.left;
                    $("#" + setting.tipID).css(tipCss).show();
                }
            });
        },
    ...
    $.fn.formValidator = function (cs){
        ...
        //先合并整个配置(深度拷贝)
        $.extend(true, setting, formValidator_setting, cs);

        var initConfig = $(doc.body).data("data-fv-group-" + setting.validatorGroup);
        ...
        return this.each(function (){
            ...
            //自动形成TIP
            var target = $(this);
            this.validatorGroup = setting.validatorGroup;
            ...
            var tipID = setting.tipID ? setting.tipID : id + "Tip";
            ...
            if (initConfig.mode === "AutoTip" || initConfig.mode === "FixTip") {
                ...
                if (initConfig.mode === "AutoTip" && tipDiv.length === 0) {
                    var relativeID = setting.relativeID ? setting.relativeID : id;
                    var $relativeID = $("#" + relativeID);
                    var offset = $relativeID.offset();
                    var tipCss = $.extend({}, setting.tipCss);
                    tipCss.top = offset.top + setting.tipCss.top;
                    tipCss.left = $relativeID.width() + offset.left + setting.tipCss.left;
                    tipDiv = $("<div style='position:absolute;' id='" + tipID + "'></div>").appendTo(document.body);
                    tipDiv.css(tipCss);
            ...
            setting.tipID = tipID;
            setting.pwdTipID = setting.pwdTipID ? setting.pwdTipID : setting.tipID;
            setting.fixTipID = setting.fixTipID ? setting.fixTipID : id + "FixTip";

            $.formValidator.appendValid(id, setting);

            //保存控件ID
            var validIndex = $.inArray(target, initConfig.validObjects);
            if (validIndex === -1) {
                if (setting.ajax) {
            ...
            if (fixTip.length && onMouseOutFixTextHtml && onMouseOnFixTextHtml && showFixText) {
                target.hover(
                    function (){
                ...
                fixTip
                    ...
                    .html(onMouseOutFixTextHtml.replace(/\$data\$/g, setting.onShowFixText));
            }
            ...
            if (srcTag === "input" || srcTag === "textarea") {
                ...
                target.focus(function (e){
                    ...
                    if (isInputControl) {
                        var val = target.val();
                        that.validValueOld = val;
                        if (showText === val) {
                            that.value = "";
                            target.css("color", setting.onShowTextColor.mouseOnColor);
                        }
                    }
                    ...
                    if (initConfig.mode !== "AlertTip") {
                        ...
                        if (!tipjq.hasClass('onError')) {
                            that.lastshowclass = tipjq.attr("class");
                            that.lastshowmsg = tipjq.text();
                            $.formValidator.setTipState(that, "onFocus", setting.onFocus);
                        }
                    }

                    if (that.validatorPasswordIndex > 0) {
                ...
                target.bind("keyup", function (){
                    ...
                    if (that.validatorPasswordIndex > 0) {
                        try {
                            ...
                            var level = $.formValidator.passwordValid(that);
                            var $pwdTipID = $("#" + setting.pwdTipID);
                            ...
                            $pwdTipID.show();
                            $pwdTipID.html(passwordStrengthStatusHtml[level]);
                        } catch (e) {
                ...
                target.bind(setting.triggerEvent, function (){
                    var that = this;
                    var settings = this.settings;
                    //根据配置截掉左右的空格
                    if (settings[0].leftTrim) {that.value = that.value.replace(/^\s*/g, "")}
                    if (settings[0].rightTrim) {that.value = that.value.replace(/\s*$/g, "")}
                    //恢复默认值
                    if (isInputControl) {
                        if (that.value === "" && showText !== "") {that.value = showText}
                        if (that.value === showText) {target.css("color", setting.onShowTextColor.mouseOutColor)}
                    }
                    //进行有效性校验
                    var returnObj = $.formValidator.oneIsValid(id);
                    ...
                    if (returnObj.ajax >= 0) {
                        ...
                        if (!returnObj.isValid) {
                            ...
                            if (auto) {
                                $(this).val(that.validValueOld);

                                if (initConfig.mode !== "AlertTip") {
                                    $.formValidator.setTipState(
                                        that, "onShow",
                                        $.formValidator.getStatusText(that, setting.onCorrect));
                                } else {
                                    initConfig.onAlert(showmsg);
                                }
                            } else {
                ...
                target.bind({
                    ...
                    blur: function (){
                        var that = this;
                        if (that.validValueOld === undefined || that.validValueOld === target.val()) {
                            $(that).trigger("change");
                        }
                    },
                    //选择项目后触发
    ...
    $.fn.ajaxValidator = function (controlOptions){
        ...
        return this.each(function (){
            var initConfig = $(doc.body).data("data-fv-group-" + this.validatorGroup);
    ...
    $.fn.defaultPassed = function (onShow){
        return this.each(function (){
            ...
            for (var i = 1; i < settings.length; i++) {
                ...
                if (!($(doc.body).data("data-fv-group-" + settings[0].validatorGroup).mode === "AlertTip")) {
    ...
    $.fn.showTooltips = function (){
        if (!$("#fv-singletip").length) {
            var singleTip = $("<div id='fv-singletip' style='position:absolute;z-index:56002'>"
                + "<div id='fv-singletip-content' class='ui-fv-singletip'></div></div>");

            $(doc.body).append(singleTip);
            singleTip.before("<iframe index='0' src='about:blank' class='fv-iframe' scrolling='no' frameborder='0'></iframe>");
        }
        ...
        return this.each(function (){
            ...
            target.bind({
                mouseenter: function (e){
                    var message = $(this).data("data-message");
                    $("#fv-singletip-content").html(message);
                    $("#fv-singletip").show();
